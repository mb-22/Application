...
export class SearchName {
    ...
    filterData (e)
    {
        // console.log("usernameTR",Data[0].innerHTML);
        //let name = Data[1].childNodes[3].children[1].innerHTML;
        //console.log(name);
        const searchedString = e.target.value.toLowerCase().replace(/\s/g, "");
        for (let i = 0 ; i < Data.length ; i++)
        {
            if (Data[i].innerHTML.toLowerCase().includes(searchedString) )
            {
                let parentOperation = document.getElementById("operation");
                parentOperation.style.display = "flex";
                Data[i].parentNode.style.display = "flex";
                // Data[i].style.display = "block";
            }
            else 
            {
                Data[i].parentNode.style.display = "none";
